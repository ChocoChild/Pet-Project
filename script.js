...
const editInput = document.querySelector('.add__input');
...
function renderTodo(todo, newPost) {
    newPost.innerHTML = todo.map((todo, index) => {
        return `<div id="div-${index}" class="wrapper__add-list">
                    <input id="input-${index}" class="add__input" readonly="readonly" type="text" value="${todo.saves}" onChange="changeTodo(event, ${index})">
                    <button class="button-1" onclick="editTodo(${index})">
...
function editTodo(index) {
    document.getElementById(`input-${index}`).removeAttribute('readonly');
    console.log(document.getElementById(`input-${index}`).value);
}


function changeTodo(event, index) {
    arrayOfTodo[index] = {
